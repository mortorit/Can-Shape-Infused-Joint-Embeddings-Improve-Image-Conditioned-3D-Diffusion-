...
def p_xt(xt, beta_schedule, gd, noise, t):
    """
    sample from x_{t-1}.

    Args:
        xt: the sample at timestep x_t
        beta_schedule: the schedule for beta
        gd: the gaussian diffusion instance
        noise: the predicted noise at timestep t
        t: the current timestep

    Returns:
        a sample from x_{t-1}
    """
    alpha_t = gd.alphas[t]
    alpha_bar_t = gd.alphas_cumprod[t]
