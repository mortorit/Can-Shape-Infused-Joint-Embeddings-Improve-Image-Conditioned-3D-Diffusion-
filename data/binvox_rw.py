...
...
>>> data_ds = binvox_rw.dense_to_sparse(md.data_im)
>>> data_sd = binvox_rw.sparse_to_dense(ms.data_im, 32)
>>> np.all(data_sd==md.data_im)
True
...
>>> np.all(ms.data_im[:, np.lexsort(ms.data_im)] == data_ds[:, np.lexsort(data_ds)])
True
"""
...
class Voxels(object):
    ...
    data_im is either a three-dimensional numpy boolean array (dense representation)
...
def read_as_3d_array(fp, fix_coords=True):
    ...
    # if just using reshape() on the raw data_im:
    # indexing the array as array[i,j,k], the indices map into the
    # coords as:
    # i -> x
    # j -> z
    # k -> y
    # if fix_coords is true, then data_im is rearranged so that
    # mapping is
    # i -> x
    # j -> y
    # k -> z
...
def read_as_coord_array(fp, fix_coords=True):
    ...
    data_im).  Note that coordinates refer to the binvox voxels, without any
    ...
    # return Voxels(data_im, dims, translate, scale, axis_order)
...
def write(voxel_model, fp):
    ...
    """
    if voxel_model.data_im.ndim == 2:
        # TODO avoid conversion to dense
        dense_voxel_data = sparse_to_dense(voxel_model.data_im, voxel_model.dims)
    else:
        dense_voxel_data = voxel_model.data_im
